...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['signup'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO users (username, email, password, address, contact, gender, state_id, city_id, country_id, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    if (!$stmt) {
        die("Database error (INSERT): " . $conn->error);
    }
    $stmt->bind_param("sssssssiii", $name, $email, $hashedPassword, $address, $contact, $gender, $state_id, $city_id, $country_id, $created_at);
    if (!$stmt->execute()) {
